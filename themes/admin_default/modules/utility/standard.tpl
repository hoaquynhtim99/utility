...
<form action="{FORM_ACTION}" method="post" name="levelnone" id="levelnone">
	<table class="tab1">
		...
		<tbody{ROW.class}>
			<tr>
				<td>
                    <select name="weight" id="weight{ROW.id}" onchange="nv_change_standard_weight({ROW.id});">
                        <!-- BEGIN: weight -->
				...
				<td class="center"><input name="status" id="change_status{ROW.id}" value="1" type="checkbox"{ROW.status} onclick="nv_change_standard_status({ROW.id})" /></td>
				<td class="center">
