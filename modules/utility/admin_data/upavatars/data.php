...
{
	$c .= '<div class="infoerror">' . $u_lang['info'] . '</div>';
}
else
{
	if( file_exists( NV_ROOTDIR . '/' . DIR_FORUM . '/nukeviet/is_user.php' ) and file_exists( NV_ROOTDIR . '/' . DIR_FORUM . '/includes/constants.' . $phpEx))
	{
		...
		if ( $nv_Request->isset_request( 'do', 'get' ) )
		{
			...
			if( $num_users <= 0 )
			{
				$c .= '<div class="infoerror">' . $u_lang['error_empty_sesion'] . '</div>';
			}
			...
			else
			{
				...
				$sql = "SELECT a.userid, a.photo, b.user_avatar, b.user_avatar_type FROM `" . NV_USERS_GLOBALTABLE . "` AS a INNER JOIN `" . USERS_TABLE . "` AS b ON a.userid=b.user_id LIMIT " . $page . "," . $per_page;
				$result = $db->sql_query( $sql );
				
				$num_ok = 0;
				$del_fail = array();
				while( list( $uid, $uimg, $uavatar, $avatar_type ) = $db->sql_fetchrow( $result ) )
				{
					...
					if( $avatar_type == AVATAR_UPLOAD )
					{
						...
						if (isset($uavatar[0]) && $uavatar[0] === 'g')
						{
							$avatar_group = true;
							$uavatar = substr($uavatar, 1);
						}
						
						if (strpos($uavatar, '.') == false) $exit = true;
						if (!$exit)
						{
							$ext		= substr(strrchr($uavatar, '.'), 1);
							$uavatar	= (int) $uavatar;
						}
						if (!$exit && !in_array($ext, array('png', 'gif', 'jpg', 'jpeg'))) $exit = true;
					...
					if( file_exists( $image_info['src'] ) )
					{
						...
						if( $db->sql_query("UPDATE `" . NV_USERS_GLOBALTABLE . "` SET `photo`=" . $db->dbescape($f_image) . " WHERE `userid`=" . $uid) )
						{
				...
				{					
					...
					$c .= '<div class="infook">' . sprintf( $u_lang['inof_prosessing'], $per_page, $page, $num_users, $num_ok ) . '</div>';
					$c .= '<meta http-equiv="refresh" content="2;url=' . $base_url_js . '&do=1&page=' . ( $page + $per_page) . '" />';
				}
				else
				{
					$c .= '<div class="infook">' . sprintf( $u_lang['inof_prosessing_ok'], $num_users ) . '</div>';
		...
		list( $num_u_web ) = $db->sql_fetchrow($db->sql_query("SELECT COUNT(*) FROM `" . NV_USERS_GLOBALTABLE . "`"));
		list( $num_u_4u ) = $db->sql_fetchrow($db->sql_query("SELECT COUNT(*) FROM `" . USERS_TABLE . "` WHERE `user_ip`!=''"));
		
		$nv_Request->set_Session( 'utility_tmp', $num_u_web );
		
		$c .= '<div class="infook">' . sprintf( $u_lang['info_num'], $num_u_web, $num_u_4u ) . '</div>';
		if( $num_u_web > 0 )
		{
	...
	{
		$c .= '<div class="infoerror">' . $u_lang['no_exist_forum'] . '</div>';
	}	
}
